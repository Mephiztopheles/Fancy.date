(function ( window, $ ) {
    ...
    FancyDate.api.create           = function () {
        ...
        if ( this.settings.max ) {
            x = Math.min( x, this.settings.max.getFullYear() );
            y = x + 50;
        }
        if ( this.settings.min ) {
            y = Math.max( y, this.settings.min.getFullYear() );
    ...
    FancyDate.api.destroy          = function () {
        this.element.off( "." + NAME ).data( NAME, null );
        this.close();
    };
    Fancy.settings[ NAME ]         = {
        format        : "dd.mm.yyyy",
